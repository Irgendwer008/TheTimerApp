...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        calendarView.setOnDateChangeListener { _, year, month, dayOfMonth ->
            ...
            val targetTime = LocalDateTime.of(year, month + 1, dayOfMonth, 0, 0)
            ...
            if (currentTime.isBefore(targetTime)) {
                ...
                countdownTimer = object : CountDownTimer(duration, 1000) {
                    override fun onTick(millisUntilFinished: Long) {
                        ...
                        if (days > 1L) {
                            runOnUiThread {
                                tv_countdown.text = "$day_string Tage, $hour_string:$minute_string:$second_string"
                            }
                        }
                        else if (days == 1L) {
                            runOnUiThread {
                                tv_countdown.text = "1 Tag, $hour_string:$minute_string:$second_string"
                            }
                        }
                        else {
                            runOnUiThread {
                                tv_countdown.text = "$hour_string:$minute_string:$second_string"
                            }
                        }
                    }
                    ...
                    override fun onFinish() {
                        ...
                        runOnUiThread {
                            tv_countdown.text = "Ziel erreicht!"
                        }
                    }
                }.start()
            } else {
                // If the selected date is in the past
                tv_countdown.text = "Datum liegt in der Vergangenheit!"
            }
        }
    }
}
